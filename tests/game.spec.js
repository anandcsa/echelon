...
test('world assets load, exposure works, movement, shooting, recharge and EMP respond',async({page})=>{
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await open(page);expect((await state(page)).version).toBe(7);await page.locator('#brightness').fill('1.3');await expect(page.locator('#brightness-value')).toHaveText('130%');await page.locator('#start').click();await expect(page.locator('#hud')).toBeVisible();const before=(await state(page)).position;await page.keyboard.down('w');await expect.poll(async()=>(await state(page)).position[2],{timeout:15000}).toBeLessThan(before[2]-.2);await page.keyboard.up('w');await page.mouse.click(450,300);await expect.poll(async()=>(await state(page)).ammo,{timeout:10000}).toBeLessThan(18);await page.keyboard.press('r');await expect.poll(async()=>(await state(page)).ammo,{timeout:20000}).toBe(18);await page.keyboard.press('q');await expect.poll(async()=>(await state(page)).emp).toBeGreaterThan(0);await page.keyboard.press('Escape');await expect(page.locator('#resume')).toBeVisible();expect(errors).toEqual([]);
});
